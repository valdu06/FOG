...
if ( $_FILES["report"] != null  )
{
	...
	{
		$uploadfile = realpath(getSetting($conn, "FOG_REPORT_DIR" )) . "/" . basename($_FILES['report']['name']); 
		if ( file_exists( getSetting($conn, "FOG_REPORT_DIR" ) ) )
		{
		...
		{
			msgBox( _("Unable to locate ") .  getSetting($conn, "FOG_REPORT_DIR" ) );
		}
	}
}
