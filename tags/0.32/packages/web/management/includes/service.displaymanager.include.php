...
<?php		
	echo ( "<form method=\"post\" action=\"?node=$_GET[node]&sub=$_GET[sub]&updatestatus=1\">" );
		echo ( "<center><table cellpadding=0 cellspacing=0 border=0 width=90%>" );
			$enabled = getSetting( $conn, "FOG_SERVICE_DISPLAYMANAGER_ENABLED" );
			$checked = "";
			if ( $enabled == "1" )
			{
				$checked = " checked=\"checked\" ";
			}
			echo ( "<tr><td width=\"270\">&nbsp;"._("Display Manager Enabled?")."</td><td>&nbsp;<input type=\"checkbox\" name=\"en\" $checked /></td><td><span class=\"icon icon-help hand\" title=\"" . _("This setting will globally enable or disable the display manager service module.  If you disable the module, it will be disabled for all clients, regardsless of the host specific setting.") . "\"></span></td></tr>" );
	...
	echo ( "<p class=\"titleBottomLeft\">"._("Default Setting")."</p>" );
		...
		echo ( "<form method=\"post\" action=\"?node=$_GET[node]&sub=$_GET[sub]&updatedefaults=1\">" );
			echo ( "<p>"._("Default width").": <input type=\"text\" name=\"width\" value=\"" . getSetting( $conn, "FOG_SERVICE_DISPLAYMANAGER_X") . "\" /></p>" );
			echo ( "<p>"._("Default height").": <input type=\"text\" name=\"height\" value=\"" . getSetting( $conn, "FOG_SERVICE_DISPLAYMANAGER_Y") . "\" /></p>" );			
			echo ( "<p>"._("Default Refresh Rate").": <input type=\"text\" name=\"refresh\" value=\"" . getSetting( $conn, "FOG_SERVICE_DISPLAYMANAGER_R") . "\" /></p>" );						
