...
/*, MouseWheelListener*/{
  ...
  public final static String PREF_MYPROXY_UNAME=
      "sshterm.myproxy.defaults.username";
  ...
  public final static String PREF_PKCS12_DEFUALT_FILE =
      "sshterm.pkcs12.defaults.file";

  public final static String PREF_BROWSER_PROFILE =
      "sshterm.browser.profile";
  public final static String PREF_BROWSER_DN =
      "sshterm.browser.dn";
  ...
  public void init(SshToolsApplication application) throws
    ...
    if(PreferencesStore.get(PREF_KRB5_MYPROXY_USE, "NONE").indexOf("true")>=0) kerb_support = true;

    //  Additional connection tabs
    if(kerb_support == true) {
	additionalTabs = new SshToolsConnectionTab[] {
	    new SshTermCommandTab(), new SshTermTerminalTab() , new GSIAuthTab(),
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (sessionActions.containsKey(event.getActionCommand())) {
      ...
      for (Iterator it = sessionFrames.iterator();
        ...
        if (action.getProvider().getProviderClass().isInstance(frame.getSessionPanel()) && frame.getSessionPanel().singleFrame()) {
          frame.show();
          return;
        }
      }
  ...
  public void connect(final SshToolsConnectionProfile profile,
	...
	if(e!=null && e instanceof java.io.EOFException && e.getMessage().indexOf("remote host has closed the connection")>=0) {
	    if(!disconnecting)
                showErrorMessage(SshTerminalPanel.this,
	    ...
	    if(!disconnecting) showExceptionMessage("GSI-SSHTerm",
