 ...
 * @version $Id: DirectoryListingTableModel.java,v 1.4 2007/10/09 14:55:39 davidspence Exp $
 */
...
public class DirectoryListingTableModel
  ...
  public boolean isDirectoryInternal(int row) {
      ...
      // Need to stop this calling stat() when re-drawing
    ...
    synchronized (lock) {
      ...
      return ((Boolean) icons.get(row)).booleanValue();
  ...
  public void refresh() throws IOException {
	...
	for (int i=0;i<files.size();i++ ) {
	    icons.add(i, new Boolean(isDirectory(i)));
	}
