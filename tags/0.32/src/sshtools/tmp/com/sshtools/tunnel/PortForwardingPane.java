...
public class PortForwardingPane
  ...
  protected String selectedTunnelName() {
    if (table.getSelectedRow() > -1) {
      try {
        ForwardingConfiguration conf = model.getForwardingConfigurationAt(table.getSelectedRow());
	return conf.getName();
      }
      catch (Exception e) {
      }
    ...
    if (option != JOptionPane.CANCEL_OPTION && option != JOptionPane.CLOSED_OPTION) {
      try {
        ...
        if (editor.isLocal()) {
          ...
          ForwardingConfiguration f = forwardingClient.addRemoteForwarding(id,
                                               editor.getBindAddress(),
                                               editor.getLocalPort(),
                                               editor.getHost(),
                                               editor.getRemotePort());
  ...
  public void startForwarding() {
    ...
    for (int i = 0; i < table.getRowCount(); i++) {
	  ...
	  if (conf.getName().equals("x11")) {
	      JOptionPane.showMessageDialog(this, "You cannot change the state of X11 forwarding.", "Error",
					    JOptionPane.ERROR_MESSAGE);
	      continue;
	  }
