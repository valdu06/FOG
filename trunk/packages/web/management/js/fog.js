 ...
 *	Revision:	$Revision: 895 $
 *	Last Update:	$LastChangedDate: 2013-01-15 20:20:15 +0000 (Tue, 15 Jan 2013) $
 ***/
...
(function($)
{
				...
				this.SearchAJAX = $.ajax({
					...
					'success':	function(response)
					{
						...
						if (response['data'].length > 0)
						{
							...
							var tr = $('tr', tbody);
							for (i in response['data']) tr.eq(i).addClass((i % 2 ? 'alt1' : 'alt2')).data({ 'id': response['data'][i]['id'], 'hostname': response['data'][i]['hostname'] });
	...
	{	
		...
		{		
			...
			// Extract hostname
			var hostname =  tr.data('hostname') || tr.attr('id') && tr.attr('id').replace(/^host-/, '') || tr.children("[id]").attr('id').replace(/^host-/, ''); 
