...
?>
	<h2><?php echo(_("Configure Auto Log Out Service Module")); ?></h2>
	
	<p class="l padded">
		<?php echo(_("The Auto Log Out service module will log a user out of a workstation after x minutes of inactivity.")); ?>
	</p>
...
<?php		
	echo ( "<form method=\"post\" action=\"?node=$_GET[node]&sub=$_GET[sub]&updatestatus=1\">" );
		echo ( "<center><table cellpadding=0 cellspacing=0 border=0 width=90%>" );
			$enabled = $GLOBALS['FOGCore']->getSetting( "FOG_SERVICE_AUTOLOGOFF_ENABLED" );
			$checked = "";
			if ( $enabled == "1" )
			{
	...
	echo ( "<p class=\"titleBottomLeft\">"._("Default Setting")."</p>" );
		...
		echo ( "<form method=\"post\" action=\"?node=$_GET[node]&sub=$_GET[sub]&updatedefaults=1\">" );
			echo ( "<p>"._("Default log out time (in minutes)").": <input type=\"text\" name=\"tme\" value=\"" . $GLOBALS['FOGCore']->getSetting( "FOG_SERVICE_AUTOLOGOFF_MIN") . "\" /></p>" );
